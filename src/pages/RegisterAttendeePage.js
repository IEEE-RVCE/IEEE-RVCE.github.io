...
export default function RegisterAttendeePage(props) {
  ...
  const submitData = () => {
    ...
    axios
      .post(hostname + '/api/attendee', values, {
        headers: {
      ...
      .then(response => {
        if (response.data.ok === true) setRegRes({ ...regRes, success: true });
        else setRegRes({ ...regRes, error: true });
      })
      .then(() => {
    ...
    <div className={classes.root}>
      <Container>
        <Paper className={classes.paper} align="center">
          ...
          <br />
          <Button onClick={submitData} style={{ margin: '1%' }}>
            Register
          </Button>
        </Paper>
      </Container>
