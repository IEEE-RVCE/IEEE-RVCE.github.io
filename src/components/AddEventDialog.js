...
export const AddEventDialog = (props) => {
  ...
  /**
   *
   * @param {*} type large or small poster
   */
  const createUpload = async (type, file) => {
    if (values.event !== "" && values.society !== "") {
      const imageRef = await ref(
        storage,
        `PosterImages/${values.ename}/${type}/${type}_Poster`
      );
      ...
      throw new Error("Please fill in the event and society fields");
    }
  };
