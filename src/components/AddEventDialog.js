...
export const AddEventDialog = (props) => {
    ...
    const submitData = () => {
        ...
        if(props.edit === true) {
            ...
            axios.post(hostname + '/api/event', values, {
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: 'Bearer ' + localStorage.getItem('atoken')
                },
    ...
    return(
        <>
            <Dialog onClose={props.onClose} open={props.open} className={classes.root}>
                ...
                <DialogContent>
                    <DialogContentText>
                        Add event details here for users to read about the event. Abide to the datatypes used in the form and use submit button to add the event.
                    </DialogContentText>
                    ...
                    </div>
                    {
                        values.hosts.map((host, i) => {
                            return (
                                <>
                                    <Typography variant='h6' className={classes.fields}>{"Host " + Number(i+1)}</Typography>
                                    <div className={classes.fields}>
                                        <TextField
                                            id={"name" + Number(i+1)}
                    ...
                    <Button 
                        onClick={
                            () =>{
                                setValues({...values, hosts: [...values.hosts, {name: '', piclink: '', details: ''}]})
                            }
                        } 
                ...
                <DialogActions>
                    <Button onClick={submitData} className={classes.button}>Submit</Button>
                </DialogActions>
            </Dialog>
            <Snackbar open={meta.error} autoHideDuration={6000} onClose={handleClose('error')}>
                <Alert elevation={6} variant="filled" onClose={handleClose('error')} severity="error">{props.edit?"An error occurred while editing an event":"An error occurred while adding event"}</Alert>
            </Snackbar>
            <Snackbar open={meta.success} autoHideDuration={6000} onClose={handleClose('success')}>
                <Alert elevation={6} variant="filled" onClose={handleClose('success')} severity="success">{props.edit?"Event edited successfully":"Event added successfully"}</Alert>
            </Snackbar>
        </>
    )
}