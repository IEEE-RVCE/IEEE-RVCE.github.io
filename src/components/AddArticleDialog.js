...
import {Dialog, AppBar, Toolbar, IconButton, Typography, Button, Slide, TextField, FormControl, InputLabel, Select, MenuItem, Snackbar} from '@material-ui/core';
...
export const AddArticleDialog = (props) => {
    ...
    const handleSubmit = () => {
        console.log(values)
        if(props.edit === true) {
            axios.put(hostname + '/api/article/' + props.data.arid, values, {
                headers: {
            ...
            axios.post(hostname + '/api/article', values, {
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: 'Bearer ' + localStorage.getItem('atoken')
                },
    ...
    return(
        <Dialog fullScreen open={props.open} onClose={props.onClose} TransitionComponent={Transition}>
            ...
            <div className={classes.root}>
                ...
                <EditArticle readOnly={false} edit={props.edit} editorContent={props.edit?props.data.content:undefined} onChange={handleContentChange}/>
            </div>
            <Snackbar open={meta.error} autoHideDuration={6000} onClose={handleClose('error')}>
                <Alert elevation={6} variant="filled" onClose={handleClose('error')} severity="error">{props.edit?"An error occurred while editing an article":"An error occurred while adding article"}</Alert>
            </Snackbar>
            <Snackbar open={meta.success} autoHideDuration={6000} onClose={handleClose('success')}>
                <Alert elevation={6} variant="filled" onClose={handleClose('success')} severity="success">{props.edit?"Article edited successfully":"Article added successfully"}</Alert>
            </Snackbar>
        </Dialog>
    )
}