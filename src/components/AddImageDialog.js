import { Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, FormControl, InputLabel, makeStyles, MenuItem, Select, Snackbar, TextField,} from '@material-ui/core'
...
export const AddImageDialog = (props) => {
    ...
    const handleFileChange = (event) => {
        ...
        reader.onload = () => {
            setValues({...values, image: {type: "Buffer", data: [...(new Uint8Array(reader.result))]}})
    ...
    const submitData = () => {
        ...
        if(props.edit === true) {
            ...
            axios.post(hostname + '/api/gallery', values, {
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: 'Bearer ' + localStorage.getItem('atoken')
                },
    ...
    return(
        <>
            ...
            <Snackbar open={meta.error} autoHideDuration={6000} onClose={handleClose('error')}>
                <Alert elevation={6} variant="filled" onClose={handleClose('error')} severity="error">{props.edit?"An error occurred while updating an image":"An error occurred while adding image"}</Alert>
            </Snackbar>
            <Snackbar open={meta.success} autoHideDuration={6000} onClose={handleClose('success')}>
                <Alert elevation={6} variant="filled" onClose={handleClose('success')} severity="success">{props.edit?"Image updated successfully":"Image added successfully"}</Alert>
            </Snackbar>
        </>
    )
}