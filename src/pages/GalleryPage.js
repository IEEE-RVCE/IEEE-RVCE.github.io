...
export default function GalleryPage(props) {
  ...
  const handleFilter = event => {
    ...
    window.location.href =
      event.target.value !== 0
        ? window.location.origin + '/#/gallery?ecat=' + event.target.value
  ...
  useEffect(() => {
    ...
    function handleResize() {
      ...
      if (window.innerWidth <= 600) setColNum(1);
      else if (window.innerWidth <= 900) setColNum(2);
      else setColNum(3);
    }
  ...
  const handleDeleteImage = iid => event => {
    axios
      .delete(hostname + '/api/gallery/' + iid, {
        headers: {
      ...
      <Container>
        ...
        <div className={classes.imgrid}>
          {tileData !== undefined && tileData.length !== 0 ? (
            <GridList cellHeight={400} cols={colnum}>
