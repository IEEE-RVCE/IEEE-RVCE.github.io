...
export default function APSSocietyPage(props) {
  ...
  useEffect(() => {
    axios.get(hostname + '/api/event/cat/' + ecats.aps).then(response => {
      setEvents(
  ...
  return (
    <div className={classes.root}>
      <img
        src={images.landing.aps}
        id="soclanding"
        alt="Antenna Propogation Society"
        ...
        {events.length !== 0 && (
          <>
            <Paper className={classes.paper}>
              ...
              <br />
              {events.length >= 4 && (
                <Link to={'/events?ecat=' + ecats.aps} className={classes.link}>
